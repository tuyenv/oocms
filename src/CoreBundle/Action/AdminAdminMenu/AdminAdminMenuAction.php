...
class AdminAdminMenuAction
{
    public static function GET(CoreAdminController $_this, Request $request)
    {
        ...
        foreach ($arr as $key => $value) {
            ...
            $action_links[] = array(
              'title' => 'Edit',
              'link' => $_this->generateUrl('admin_admin_menu_edit_page', array('menuId' => $value['id'])),
            );
            $action_links[] = array(
              'title' => 'Add New Link',
              'link' => $_this->generateUrl('admin_admin_menu_link_edit_page', array('menuId' => $value['id'], 'linkId' => 0)),
            );
